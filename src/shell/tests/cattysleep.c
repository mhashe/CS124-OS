 ...
 * for a specified number of seconds after doing so.
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        printf("\tBehaves like cat, then sleeps for specified number of"
               " seconds\n");
        return 1;
    }
    ...
    /* Since we will still sleep for some amount of time, we need to
     * close these so that any other processes in the command will
     * see EOFs from us.
     */
    close(STDIN_FILENO);
    close(STDOUT_FILENO);
