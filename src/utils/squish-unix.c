...
/*!
 * Make FD non-blocking if NONBLOCKING is true, or blocking if NONBLOCKING
 * is false.
 */
...
/*!
 * Handle a read or write on *FD, which is the socket if FD_IS_SOCK is true,
 * that returned end-of-file or error indication RETVAL.  The system call is
 * named CALL, for use in error messages.  Returns true if processing may
 * continue, false if we're all done.
 */
...
static void relay(int sock) {
    ...
    /* Configure noncanonical mode on stdin to avoid waiting for end-of-line. */
    ...
    while (pipes[0].in != -1 || pipes[1].in != -1 ||
        ...
        for (i = 0; i < 2; i++) {
            ...
            /* Don't do anything with the stdin->sock pipe until we have some
             * data for the sock->stdout pipe.  If we get too eager, vmplayer
             * will throw away our input.
             */
