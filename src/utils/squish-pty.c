...
/*!
 * Prints MSG, formatting as with printf(), plus an error message based on
 * errno, and exits.
 */
static void fail_io(const char *msg, ...) {
    va_list args;
...
/*!
 * If FD is a terminal, configures it for noncanonical input mode with VMIN
 * and VTIME set as indicated.  If FD is not a terminal, has no effect.
 */
...
/*!
 * Make FD non-blocking if NONBLOCKING is true, or blocking if NONBLOCKING is
 * false.
 */
...
/*!
 * Handle a read or write on *FD, which is the pty if FD_IS_PTY is true, that
 * returned end-of-file or error indication RETVAL.
 *
...
static void relay(int pty, int dead_child_fd) {
    ...
    /* Configure noncanonical mode on PTY and stdin to avoid waiting for
     * end-of-line.  We want to minimize context switching on PTY (for
     * efficiency) and minimize latency on stdin to avoid a laggy user
     * experience.
     */
    ...
    while (pipes[1].in != -1) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            /* Don't do anything with the stdin->pty pipe until we have some
             * data for the pty->stdout pipe.  If we get too eager, Bochs will
             * throw away our input.
             */
...
int main(int argc __attribute__ ((unused)), char *argv[]) {
    ...
    /* Arrange to get notified when a child dies, by writing a byte to a pipe
     * fd.  We really want to use pselect() and sigprocmask(), but Solaris 2.7
     * doesn't have it.
     */
    if (pipe(pipe_fds) < 0)
        fail_io ("pipe");
    ...
    /* Save the virtual interval timer, which might have been set by the
     * process that ran us.  It really should be applied to our child process.
     */
    ...
    if (pid < 0) {
        ...
        /* If the subprocess has died, die in the same fashion.  In particular,
         * dying from SIGVTALRM tells the pintos script that we ran out of CPU
         * time.
         */
